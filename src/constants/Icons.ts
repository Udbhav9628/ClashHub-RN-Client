const cross = require("../Assets/icons/cross.png");
const EyeOpen = require("../Assets/icons/Eyeopen.png");
const EyeClose = require("../Assets/icons/Eyeclose.png");
const Incorrect = require("../Assets/icons/Incorrect.png");
const Correct = require("../Assets/icons/correct.png");
const Goggle = require("../Assets/icons/google.png");
const menu = require("../Assets/icons/menu.png");
const search = require("../Assets/icons/search.png");
const filter = require("../Assets/icons/filter.png");
const down_arrow = require("../Assets/icons/down_arrow.png");
const calories = require("../Assets/icons/calories.png");
const logout = require("../Assets/icons/logout.png");
const notification = require("../Assets/icons/notification.png");
const Clock = require("../Assets/icons/clock.jpg");
const profile = require("../Assets/icons/profile.png");
const setting = require("../Assets/icons/setting.png");
const wallet = require("../Assets/icons/wallet.png");
const help = require("../Assets/icons/help.png");

// Images
const Logo = require("../Assets/Images/App_Logo.png");
const Crousal = require("../Assets/Images/crousal1.jpg");
const Pubg = require("../Assets/Images/Pubg.jpg");
const Pubg2 = require("../Assets/Images/pubg.png");
const FreeFire = require("../Assets/Images/FreeFire.jpg");
const Codm = require("../Assets/Images/Codm.png");
const BGMI = require("../Assets/Images/BGMI.jpg");
const Fortnite = require("../Assets/Images/Fortnite.jpg");
const Crad = require("../Assets/Images/card.png");

export default {
  cross,
  EyeOpen,
  EyeClose,
  Incorrect,
  Correct,
  Goggle,
  menu,
  search,
  filter,
  down_arrow,
  calories,
  logout,
  notification,
  Clock,
  profile,
  setting,
  wallet,
  help,
  Logo,
  Crousal,
  Pubg,
  Pubg2,
  FreeFire,
  Codm,
  BGMI,
  Fortnite,
  Crad,
};
